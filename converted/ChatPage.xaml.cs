...
namespace SmartKeyboard.Styles
{
    ...
    public partial class ChatPage : UserControl
    {
        ...
        private void LoadIcons()
        {
            ...
            // Load Location GIF for color boxes - السطور 277-282
            LoadAnimatedGif(OpenColorGif, Path.Combine(iconsDir, "Location.gif"));
            LoadAnimatedGif(ClosedColorGif, Path.Combine(iconsDir, "Location.gif"));
        }

        // ═══════════════════════════════════════════════════════════════
        // Load animated GIF - مطابق لـ QMovie في السطور 279-282
        // ═══════════════════════════════════════════════════════════════
        private void LoadAnimatedGif(System.Windows.Controls.MediaElement mediaElement, string path)
        {
            try
            {
                if (File.Exists(path))
                {
                    // تحميل الـ GIF المتحرك - السطور 279-282
                    mediaElement.Source = new Uri(path, UriKind.Absolute);
                    mediaElement.MediaEnded += (s, e) =>
                    {
                        // إعادة تشغيل الـ GIF في حلقة لانهائية
            ...
            {
                Console.WriteLine($"[CHAT] GIF load error: {ex.Message}");
            }
        }
        ...
        // ═══════════════════════════════════════════════════════════════
        // pick_color - السطور 350-420 (using EyedropperOverlay)
        // ═══════════════════════════════════════════════════════════════
        private void PickColor(string target)
        {
            // إنشاء أداة المصاصة
            var eyedropper = new EyedropperOverlay();
            
            // عند التقاط اللون
            eyedropper.ColorPicked += (color, x, y) =>
            {
                Dispatcher.Invoke(() =>
                {
                    if (target == "open")
                    {
                        OnOpenColorChanged(color, x, y);
                    }
                    else
                    {
                        OnClosedColorChanged(color, x, y);
                    }
                });
            };

            // عند الإلغاء - خيار بديل: فتح ColorDialog
            eyedropper.Cancelled += () =>
            {
                Dispatcher.Invoke(() =>
                {
                    // اختياري: فتح ColorDialog كبديل
                    ...
                    if (dialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                    {
                        ...
                        if (target == "open")
                        {
                            OnOpenColorChanged(pickedColor, cursorPos.X, cursorPos.Y);
                        }
                        else
                        {
                            OnClosedColorChanged(pickedColor, cursorPos.X, cursorPos.Y);
                        }
                    }
                });
            };
        ...
        private void TestButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (_openPosX < 0 || _openPosY < 0)
            {
                System.Windows.MessageBox.Show("إحداثيات غير صالحة!", "خطأ", MessageBoxButton.OK);
                return;
            }
            ...
            try
            {
                ...
                if (hDC == IntPtr.Zero)
                {
                    System.Windows.MessageBox.Show("فشل في الحصول على DC!", "خطأ", MessageBoxButton.OK);
                    return;
                }

                uint pixel = GetPixel(hDC, _openPosX, _openPosY);

                // تحويل COLORREF (0x00BBGGRR) إلى RGB
